...
export const POST = async (req: NextRequest) => {
  try {
    ...
    if (post.userId !== user!.id) {
      return NextResponse.json({ error: "You are not the owner of this post" }, { status: STATUS_CODES.UNAUTHORIZED })
    }
    ...
    return NextResponse.json({ message: "Post deleted successfully" }, { status: STATUS_CODES.SUCCESS })
  } catch (error) {
...
export type DeletePostAPIResponse = APIResponse<ReturnType<typeof POST>>
